...
// const TRADE_KPIS   = ["Export (Mt)", "Import (Mt)"];
...
//   if (kpi === "Export (Mt)") return "export";
//   if (kpi === "Import (Mt)") return "import";
//   return null;
// };
...
//     ...slideYears.map(y => y >= HIST_START && y <= HIST_END ? tc(yoyByYear[y] ?? null) : null)];
//   const fcRow   = [ts(`Forecast (${FC_START}–${FC_END})`),
//     ...slideYears.map(y => y >= FC_START && y <= FC_END ? tc(yoyByYear[y] ?? null) : null)];
//   return [
...
//       // Deduplicate — don't add Export/Import if already in global cement KPIs
//       const existingKpis = r.data.kpis;
//       const tradeOnly = TRADE_KPIS.filter(t => !existingKpis.includes(t));
//       const allKpis = [...existingKpis, ...tradeOnly];
//       setCountries(r.data.countries);
//       setKpis(allKpis);
//       setKpi(existingKpis.includes(DEFAULT_KPI) ? DEFAULT_KPI : existingKpis[0] ?? "");
...
//                   barLabel={isTradeKpi ? kpi.split(" ")[0] : "Demand"}
//                 />
//               ) : (
//                 <div style={{
