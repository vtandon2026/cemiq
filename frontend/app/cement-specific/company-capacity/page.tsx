    ...
    apiFetch("/cement-specific/company-capacity/chart", { statuses, countries, top_n: topN, min_capacity: minCapacity })
      .then(res => {
        ...
        setChartCtx({
          ...
          total_companies: res.data?.length,
          top_10: (res.data ?? []).slice(0, 10).map((r: CompanyRow) => ({
            rank: r.rank, company: r.company, capacity: r.total_capacity, share: `${r.market_share}%`, countries: r.country_count,
          })),
        });
      })
        ...
        <div style={{ flex: 1, minWidth: 0, display: "flex", gap: 16 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: "flex", gap: 12, marginBottom: 12 }}>
              {[
                { label: "Companies shown", value: chartData.length },
                { label: "Total capacity", value: `${totalCap.toFixed(0)} Mt` },
                { label: "Top 3 share", value: `${top3Share.toFixed(1)}%` },
              ].map(k => (
                <div key={k.label} style={{ background: "#fff", border: "1px solid #e9ecef", borderRadius: 8, padding: "8px 16px", boxShadow: "0 1px 4px rgba(0,0,0,0.04)", flex: 1, textAlign: "center" }}>
            ...
            <div style={{ background: "#fff", border: "1px solid #e9ecef", borderRadius: 10, padding: 16, boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                ...
                <ChartActions onCsv={downloadCsv} csvDisabled={chartData.length === 0} />
              </div>
                ...
                <div style={{ borderTop: "1px solid #f1f5f9" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11, fontFamily: F }}>
                    ...
                    <tbody>
                      {chartData.map((row, i) => (
                        <>
                          <tr key={row.company} style={{ background: i % 2 === 0 ? "#fff" : "#fafafa" }}>
                          ...
                          {expanded === row.company && (
                            <tr key={`${row.company}-detail`} style={{ background: "#fafafa" }}>
